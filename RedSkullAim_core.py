...
HOTKEY_ORDER = [f"F{i}" for i in range(1, 11)]
MODE_OPTIONS = [
    ("NONE", "ไม่ตั้งค่า"),
    ("AR", "ปืนกล (AR)"),
    ("SG", "ลูกซอง (SG)"),
    ("SGBUFF", "ลูกซอง (SG) มี Buff"),
    ("SGBERET", "ลูกซอง (SG) มีเบเร่ต์"),
    ("SNIPER", "สไนเปอร์ (Sniper)"),
    ("SNIPERBUFF", "สไนเปอร์ (Sniper) มี Buff"),
    ("SNIPERBERET", "สไนเปอร์ (Sniper) มีเบเร่ต์"),
    ("KAR98", "ปืน Kar98TSR"),
    ("KAR98BUFF", "ปืน Kar98TSR มี Buff"),
    ("KAR98BERET", "ปืน Kar98TSR มีเบเร่ต์"),
    ("CUSTOM_AR", "Custom ปืนกล (AR) ดีเลย์เอง"),
    ("CUSTOM_SG", "Custom ลูกซอง (SG) ดีเลย์เอง"),
    ("CUSTOM_SNIPER", "Custom สไนเปอร์ (Sniper) ดีเลย์เอง"),
]
MODE_LABELS = {mode_id: label for mode_id, label in MODE_OPTIONS}
MODE_DISPLAY_LABELS = [f"{label} [{mode_id}]" for mode_id, label in MODE_OPTIONS]
DISPLAY_TO_MODE = {
    f"{label} [{mode_id}]": mode_id for mode_id, label in MODE_OPTIONS
}
MODE_TO_DISPLAY = {
    mode_id: f"{label} [{mode_id}]" for mode_id, label in MODE_OPTIONS
}
DEFAULT_HOTKEY_ASSIGNMENTS = {
    "F1": "AR",
    "F2": "SG",
    "F3": "SNIPER",
    "F4": "NONE",
...
hotkey_assignments = DEFAULT_HOTKEY_ASSIGNMENTS.copy()
...
def get_hotkey_lines(assignments=None):
    assigned_modes = hotkey_assignments if assignments is None else assignments
    lines = []
    for key in HOTKEY_ORDER:
        mode_id = assigned_modes.get(key, "")
        label = MODE_LABELS.get(mode_id, mode_id or "ไม่ตั้งค่า")
        lines.append(f"{key.ljust(3)} = {label}")
    lines.extend([
        "F11 = หยุด (Pause)",
        "F12 = ออก (Exit)",
...
def schedule_self_delete() -> bool:
    """Schedule deletion of current script/exe after this process exits."""
    if getattr(sys, "frozen", False):
        target_path = os.path.abspath(sys.executable)
    else:
        entry_path = os.environ.get("REDSKULL_ENTRY_PATH", "").strip()
    ...
    temp_dir = os.environ.get("TEMP") or os.path.dirname(target_path)
    bat_path = os.path.join(temp_dir, f"rsk_delete_{int(time.time() * 1000)}.bat")
    bat_script = (
        "@echo off\r\n"
        "set \"TARGET=%~1\"\r\n"
        "if \"%TARGET%\"==\"\" exit /b\r\n"
    ...
    try:
        ...
        subprocess.Popen(["cmd", "/c", bat_path, target_path], creationflags=0x08000000)
        return True
    except Exception as exc:
        print(f"Failed to schedule self-delete: {exc}")
        return False
...
def select_resolution():
    """Show setup window for capture presets, hotkey mapping, and custom delays."""

    pending_assignments = hotkey_assignments.copy()
    hotkey_vars = {}
    delay_defaults = {
        "AR": f"{custom_delay_settings['AR']:.2f}",
        "SG": f"{custom_delay_settings['SG']:.2f}",
        "SNIPER": f"{custom_delay_settings['SNIPER']:.2f}",
    }

    def parse_delay(raw_value, gun_name):
        try:
            value = float(raw_value.strip().replace(",", "."))
        except ValueError:
            messagebox.showerror("ค่า Delay ไม่ถูกต้อง", f"{gun_name} ต้องเป็นตัวเลขเท่านั้น")
            return None
        if value < 0:
            messagebox.showerror("ค่า Delay ไม่ถูกต้อง", f"{gun_name} ต้องมากกว่าหรือเท่ากับ 0")
            return None
        return value

    def sync_pending_assignments():
        for key, var in hotkey_vars.items():
            mode_id = DISPLAY_TO_MODE.get(var.get())
            if mode_id:
                pending_assignments[key] = mode_id

    def on_select():
        global selected_region, hotkey_assignments
        choice = combo.get()
        selected_region = RESOLUTION_PRESETS.get(choice, {})
        if not selected_region:
            messagebox.showerror("ไม่พบ Preset", "กรุณาเลือกความละเอียดหน้าจอ")
            return

        ar_delay = parse_delay(delay_vars["AR"].get(), "Delay ปืนกล (AR)")
        sg_delay = parse_delay(delay_vars["SG"].get(), "Delay ลูกซอง (SG)")
        sniper_delay = parse_delay(delay_vars["SNIPER"].get(), "Delay สไนเปอร์ (Sniper)")
        if None in (ar_delay, sg_delay, sniper_delay):
            return

        sync_pending_assignments()
        hotkey_assignments = pending_assignments.copy()
        custom_delay_settings["AR"] = ar_delay
        custom_delay_settings["SG"] = sg_delay
        custom_delay_settings["SNIPER"] = sniper_delay
        root.destroy()
    ...
    def update_preview(*_):
        ...
        hotkey_preview_value.set("\n".join(get_hotkey_lines(pending_assignments)))
    ...
    delay_vars = {
        "AR": tk.StringVar(master=root, value=delay_defaults["AR"]),
        "SG": tk.StringVar(master=root, value=delay_defaults["SG"]),
        "SNIPER": tk.StringVar(master=root, value=delay_defaults["SNIPER"]),
    }
    ...
    style.configure("Dark.TEntry", fieldbackground="#1f2740", foreground="#ffffff")
    ...
    ttk.Label(
        header,
        text="Setup ใหม่: เลือกขนาดหน้าจอ + ตั้งค่าปุ่ม F1-F10 + ใส่ดีเลย์ Custom แล้วเริ่มใช้งานได้ทันที",
    ...
    ttk.Label(hotkey_card, text="Hotkeys Mapping (F1-F10)", style="CardTitle.TLabel").pack(anchor="w")
    ttk.Label(
        hotkey_card,
        style="Hint.TLabel",
        text="เลือกได้ว่า F1-F10 จะเป็นโหมดไหน ",
    ).pack(anchor="w", pady=(2, 10))

    mapping_rows = ttk.Frame(hotkey_card, style="Card.TFrame")
    mapping_rows.pack(fill="x", pady=(0, 10))

    for hotkey in HOTKEY_ORDER:
        row = ttk.Frame(mapping_rows, style="Card.TFrame")
        row.pack(fill="x", pady=3)
        ttk.Label(row, text=hotkey, style="Body.TLabel", width=5).pack(side="left")
        selected_mode = pending_assignments.get(hotkey, MODE_OPTIONS[0][0])
        var = tk.StringVar(value=MODE_TO_DISPLAY.get(selected_mode, MODE_DISPLAY_LABELS[0]))
        box = ttk.Combobox(
            row,
            values=MODE_DISPLAY_LABELS,
            state="readonly",
            textvariable=var,
        ...
        box.bind("<<ComboboxSelected>>", update_preview)
        hotkey_vars[hotkey] = var

    hotkey_preview_value = tk.StringVar(value="\n".join(get_hotkey_lines(pending_assignments)))
    tk.Label(
        hotkey_card,
        textvariable=hotkey_preview_value,
        anchor="w",
        justify="left",
        fg="#8fe3b5",
        bg="#10131e",
    ...
    ttk.Label(preset_card, text="Custom Delay (วินาที)", style="CardTitle.TLabel").pack(anchor="w", pady=(2, 4))
    ttk.Label(
        preset_card,
        text="ใช้กับโหมด CUSTOM_* ที่เลือกใน F1-F10",
        style="Hint.TLabel",
    ...
    for key in ("AR", "SG", "SNIPER"):
        delay_row = ttk.Frame(preset_card, style="Card.TFrame")
        delay_row.pack(fill="x", pady=3)
        ttk.Label(delay_row, text=delay_labels[key], style="Body.TLabel", width=20).pack(side="left")
        ttk.Entry(delay_row, textvariable=delay_vars[key], style="Dark.TEntry", width=12, font=("Consolas", 11)).pack(side="left")
        ttk.Label(delay_row, text="sec", style="Hint.TLabel").pack(side="left", padx=(6, 0))
...
def CUSTOM_AR():
    ...
    if detect_red_pixel_bgr(img):
        ...
        time.sleep(0.01)
        mouse.release("left")
        time.sleep(max(0.0, custom_delay_settings["AR"]))
...
def CUSTOM_SG():
    ...
    if detect_red_pixel_bgr(img):
        ...
        keyboard.press("q")
        mouse.wheel(-1)
        time.sleep(max(0.0, custom_delay_settings["SG"]))
        keyboard.release("q")
...
def CUSTOM_SNIPER():
    ...
    if detect_red_pixel_bgr(img):
        ...
        keyboard.press("q")
        mouse.wheel(-1)
        time.sleep(max(0.0, custom_delay_settings["SNIPER"]))
        keyboard.release("q")
...
MODE_HANDLERS.update({
    ...
    "SGBERET": SGBERET,
    "SNIPER": SNIPER,
    "SNIPERBUFF": SNIPERBUFF,
    "SNIPERBERET": SNIPERBERET,
    "KAR98": KAR98,
    "KAR98BUFF": KAR98BUFF,
    "KAR98BERET": KAR98BERET,
    "CUSTOM_AR": CUSTOM_AR,
    "CUSTOM_SG": CUSTOM_SG,
    "CUSTOM_SNIPER": CUSTOM_SNIPER,
})
...
def check_keys():
    ...
    for hotkey in HOTKEY_ORDER:
        if keyboard.is_pressed(hotkey):
            mode_id = hotkey_assignments.get(hotkey)
            handler = MODE_HANDLERS.get(mode_id)
            if handler:
                current_function = handler
                is_paused = False
                print(f"{hotkey} -> {MODE_LABELS.get(mode_id, mode_id)} started.")
